...
type Options struct {
	...
	Datastore            string `long:"datastore" description:"[postgres|prometheus|json]" env:"PW2_DATASTORE"`
...
func main() {
	...
	if !opts.Ping {
		...
		if opts.BatchingDelayMs > 0 && opts.Datastore != DATASTORE_PROMETHEUS {
			...
			go MetricsBatcher(opts.Datastore, opts.BatchingDelayMs, buffered_persist_ch, persist_ch)
		}

		if opts.Datastore == DATASTORE_JSON {
			...
			log.Fatal("Unknown datastore. Check the --datastore param [postgres|prometheus|json]")
		}
